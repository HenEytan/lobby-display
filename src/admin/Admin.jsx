        ...
        <div className="f-row">
          <label>רקע:
            <select value={banner.bg || "art_sunset"} onChange={(e) => onChange({ bg: e.target.value })}>
              ...
              <option value="school">🖼 פתיחת שנת הלימודים</option>
      ...
      <div className="sec-head">
        <div>
          ...
          <p>
            ...
            ומוצג במסך הראשי החל משלושה ימים לפני תחילתו ועד סופו. מערב החג ועד סופו הוא הבאנר היחיד בסבב — שאר הבאנרים יורדים. בימי שבתון של החג עצמו המסך עובר לתצוגה סטטית מלאה. אין צורך לתחזק תאריכים ידנית.
          </p>
        </div>
      </div>
...
function AnnouncementsTab({ data }) {
  ...
  const add = () => save([{
    ...
    pinned: false, urgent: false, created: today(), days: 7,
  }, ...anns]);
    ...
    <section>
      <div className="sec-head">
        <div>
          ...
          <p>כותבים הודעה — וכבר על המסך. בוחרים כמה ימים תוצג, נעיצה, והודעה דחופה שתופסת את המסך המלא.</p>
        </div>
          ...
          <div className="item-card ann-card" key={a.id}>
            <div className="item-fields">
              ...
              <div className="f-row">
                ...
                <label>מוצג למשך:
                  <input type="number" min="0" max="365" style={{ width: 70 }} value={a.days ?? ""}
                    onChange={(e) => update(a.id, {
                      days: e.target.value === "" ? "" : Number(e.target.value),
                      created: a.created || today(),
      ...
      <div className="settings-grid">
        ...
        <div className="item-card">
          ...
          <label className="switch">
            ...
            <span>הצגת אירועי הוד השרון בעמודה הצדדית</span>
          </label>
          <label className="switch">

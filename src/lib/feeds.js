...
// בעת נפילת המקור מוחזר ערך אחרון שנשמר / ברירת מחדל.
...
export async function fetchWeather() {
  ...
  try {
    ...
    return {
      current: { temp: 28, desc: "בהיר", code: 0 },
      days: [],
    };
  }
}

// אייקון מזג אוויר פשוט לפי קוד WMO
...
// חדשות — כותרות דוגמה. בייצור: הזנת RSS רשמית של גוף חדשות (כותרת + מקור + קישור בלבד).
export const NEWS_HEADLINES = [
  { source: "חדשות", title: "עיריית הוד השרון מקדמת תוכנית פיתוח חדשה למרכז העיר" },
  { source: "חדשות", title: "פסטיבל הקיץ העירוני יוצא לדרך עם עשרות אירועי תרבות" },
  { source: "חדשות", title: "שיפור מערך התחבורה הציבורית בשכונות החדשות" },
  { source: "חדשות", title: "פארק אקולוגי חדש ייפתח לקהל בסוף החודש" },
];

...
// ימי צום, יום הזיכרון ויום השואה לא מקבלים ברכה חגיגית (מוחזר null).
//
// הכתיב חייב להיות זה ש-hebcal מחזיר אחרי הסרת הניקוד, ולא הכתיב המלא:
// "יוֹם כִּפּוּר" יוצא "יום כפור" (בלי יו״ד), "סֻכּוֹת" יוצא "סכות" ו"יוֹם
// הַזִּכָּרוֹן" יוצא "יום הזכרון". הרשימה הקודמת חיפשה "יום כיפור" ו"סוכות"
// ולכן לא זיהתה אותם לעולם — ביום כיפור סרגל המצב הציג "בוקר טוב" במקום
// "גמר חתימה טובה", וכך גם בסוכות.
//
// "תענית בכורות" אינה ברשימה במכוון: היא חלה בערב פסח ואסור לה לבטל את
// ברכת החג. "תענית אסתר" כן — היא חלה בערב פורים, והיום הוא יום צום.
const SOLEMN = /צום|תענית אסתר|תשעה באב|עשרה בטבת|שבעה עשר בתמוז|יום הזכרון|יום הזיכרון|יום השואה/;

// הברכה עצמה נגזרת מהגדרות באנרי החגים (HOLIDAY_BANNER_DEFS) — טבלת ביטויים
// אחת לשני המקומות, כדי שכתיב שמתוקן בצד אחד לא יישאר שבור בצד השני. זו בדיוק
// התקלה שהייתה כאן: שתי רשימות שהתפצלו, ורק אחת מהן עודכנה.
function festiveGreeting(desc) {
  if (GREETING_SKIP.test(desc)) return null;
  // חלק מההגדרות מעוגנות ב-"^" כדי להבדיל בין "פסח" ל"פסח שני" ובין "סכות"
  // ל"ערב סכות" בטווח הבאנר; לברכה היומית מסירים את הקידומת "ערב " כדי
  // שהעוגן לא יחסום דווקא את ערב החג, שבו הברכה הכי במקום.
  const name = desc.replace(/^ערב /, "");
  // שמיני עצרת ושמחת תורה חולקים הגדרה עם סוכות אך אינם "חג סוכות שמח"
  if (/שמיני עצרת|שמחת תורה/.test(name)) return "חג שמח";
  for (const def of HOLIDAY_BANNER_DEFS) {
    if (def.solemn) continue;
    if (def.re.test(name)) return def.title;
  }
  return null;
}

// הברכה עולה גם ב"ערב חג": בערב יום כיפור או בערב ראש השנה המסך כבר מאחל
// לדיירים, באותה רוח שבה באנר החג עולה שלושה ימים לפני החג.
export function todayHoliday(d) {
  try {
    const events = HebrewCalendar.calendar({
      start: d, end: d, il: true, noModern: false, sedrot: false, candlelighting: false,
    });
    const names = events.map((ev) => stripNiqqud(ev.render("he")));
    // יום שיש בו צום או אזכרה אינו יום של ברכה — גם אם הוא במקביל ערב חג.
    // הבדיקה על כל אירועי היום ולא על הראשון שבהם: הסדר שבו hebcal מחזיר
    // אותם אינו מובטח.
    if (names.some((n) => SOLEMN.test(n))) return null;
    for (const n of names) {
      const greeting = festiveGreeting(n);
...
const HOLIDAY_BANNER_DEFS = [
  ...
  { key: "shoah", re: /יום השואה/, title: "יום הזיכרון לשואה ולגבורה", subtitle: "זוכרים את השישה מיליון — יהי זכרם ברוך", bg: "holiday_shoah", noEve: true, solemn: true },
  { key: "zikaron", re: /יום הזכרון|יום הזיכרון/, title: "יום הזיכרון לחללי מערכות ישראל", subtitle: "זוכרים ומוקירים את הנופלים ואת נפגעי פעולות האיבה — יהי זכרם ברוך", bg: "holiday_zikaron", solemn: true },
];
...
const NOT_THE_HOLIDAY = [
  "למעשר", "לבהמ", "רבין", "הרצל", "בן.?גוריון", "ז'בוטינסקי",
  "טרומפלדור", "יום העליה", "יום המשפחה", "סיגד", "פורים קטן", "שושן פורים",
];
// לברכה היומית "ערב חג" הוא כן יום של ברכה, ולכן "ערב " מסונן מטווחי הבאנרים
// בלבד — שם הוא היה מקדים את תחילת החג ביום נוסף מעבר ל-BANNER_LEAD_DAYS.
const GREETING_SKIP = new RegExp(NOT_THE_HOLIDAY.join("|"));
const BANNER_SKIP = new RegExp(["ערב ", ...NOT_THE_HOLIDAY].join("|"));
...
export function holidayBannerSchedule(now = new Date()) {
  const start = new Date(now);
  start.setHours(0, 0, 0, 0);
  // 13 חודשים ולא 12: חג שזה עתה הסתיים ומופעו הבא נופל מעט אחרי סוף החלון
  // נופל מהלוח לגמרי — ראש השנה נעלם מרשימת הבאנרים בדיוק אחרי ראש השנה.
  const end = new Date(start);
